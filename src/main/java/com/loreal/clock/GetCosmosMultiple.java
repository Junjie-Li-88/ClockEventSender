...
@RestController
public class GetCosmosMultiple {
    ...
    @RequestMapping(value = "/getCosmosMultiple", produces = "application/json;charset=utf-8")
    public String handleReturn(@RequestBody String parameters, HttpServletResponse response) throws ExecutionException, InterruptedException {
        ...
        if (cosmosdb == null && cosmosAsyncDb == null) {
            ...
            try {
                ...
                return "error connecting to specified contaifner: " + e;
            }
        }
    ...
    public JSONArray sendIt(JSONArray array) throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < array.size(); i++) {
            try {
                ...
                Callable c = new CallableReadCosmos(o, cosmosAsyncCont, null, false);
        ...
        if (array.size() > ret.size()) {
            System.out.println("cosmos read not ok: requested " +array.size() + " but got only " + ret.size());
        }
        return ret;
    }
